...
import { Nav, NavFooter, NavHeader, NavList, CloseNav, NavVideos } from '../styles/navigationStyles'
        ...
        <>
            ...
            <Container>
                ...
                <NavList>
                    <ul>
                        { navRoutes.map( route => (
                            <motion.li key={ route.id } onHoverStart={ () => setRevealVideo( {
                                show: true,
                                video: route.video,
                                key: route.id
                            } ) }
                            ...
                            <Link to={`/projects${route.path}`}>
                                    <motion.div initial={ { x: -108 } } whileHover={ {
                                        x: -40,
                                        transition: {
                                            duration: 0.4,
                                            ease: [0.6, 0.05, -0.01, 0.9]
                                        }
                                }} className="link" onMouseEnter={() => onCursor('pointer')} onMouseLeave={onCursor}>
